    ...
    public function index()
    {
        ...
        if($this->request->is('POST')){
            if(!empty($this->request->getData('categoria_id')))
                $estabelecimentos->andWhere(['Estabelecimentos.categoria_id'
                =>$this->request->getData('categoria_id')]);
        }
        foreach ($estabelecimentos as $estabelecimento){
            $quantidade = $this->Estabelecimentos->Avaliacaos->find('all')
                ->where(['Avaliacaos.estabelecimento_id'=>$estabelecimento->id])->count();
            $estabelecimento->qntCar = $this->Estabelecimentos->EstabelecimentoCaracteristicas->find('all')
                ->where(['EstabelecimentoCaracteristicas.estabelecimento_id'=>$estabelecimento->id])->count();
            $somatorio = 0;
            foreach ($estabelecimento->avaliacaos as $avaliacao)
                $somatorio+=$avaliacao->nota;
            $estabelecimento->media = ($quantidade == 0)?$somatorio/1 : $somatorio/$quantidade;
            $estabelecimento->qntAv = $quantidade;
        }
